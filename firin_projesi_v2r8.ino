//v2r8 revlist
//Elektrik Kesintisi hali için rampa düzenlendi
//Pot maks. sıcaklık ayarı 1100C olarak güncellendi.
...
int t_ramp_pot_old;
int t_ramp_start;                                       //t_ramp için başlangıç sıcaklık değeri
int t_time_pot;                                       // Pişirim süresi Pot okunan değer
int t_time_pot_old;
int t_max;                                            // Maksimum sıcaklık
int t_ramp;                                           // Sıcaklık rampası
float t_ramp_basamak;                                   // Rampa basamak değeri
float t_ramp_anlik;                                  // Rampanın o anki sıcaklığı
int t_olcum_periyot = 0;                            // Rampanın ölçüm periyodu (60 => dakikadada bir; 120 => 30sn'de bir)
...
unsigned long t_kontrol_zaman= 0;                 // Sıcaklık kontrol zamanları ms
unsigned long t_kontrol_zaman_periyot = 5000;        // on/off kontrol ve rampa için periyot(ms)
...
//  }
  ...
  switch (menu_sayac)
  {
    case 1: //maksimum sıcaklık değeri giriş ekranı
      ...
      if (t_max>1100)                                 // Sıcaklık değerinin 100 ile 1200 arasında olması için
      {
        t_max=1100;
      }
      else if (t_max<100)
...
{
//Saat Dakika Sayacı ve ilk değerlerin elde edilmesi
  if (t_pisirim_baslangic == 0)
  {
  t_pisirim_baslangic = millis();
                                    
  t_olcum_periyot = t_kontrol_zaman_periyot / 1000;                   //Sn cinsine çevrildi
  t_ramp_basamak = (t_olcum_periyot * (t_ramp) / (float)3600);        // Basamak adımı belirle
  t_ramp_anlik = analogRead(A3);                                      //rampa başlangıç sıcaklığı 
  t_ramp_anlik = (t_ramp_anlik*2-250);  // Gerçek değere çevir
  }
...
//Sabit Metinler
  ...
  t_instant = (t_instant_analog*2-250);  // Gerçek değere çevir
  ...
  {
    t_kontrol_zaman = t_kontrol_zaman + t_kontrol_zaman_periyot;
